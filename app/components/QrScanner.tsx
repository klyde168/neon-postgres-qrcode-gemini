// app/components/QrScanner.tsx (Updated with WebSocket support)
...
export default function QrScanner() {
  ...
  useEffect(() => {
    addDebugMessage(`WebSocket status: ${wsStatus}`);
  }, [wsStatus, addDebugMessage]);

  useEffect(() => {
    if (fetcher.data) {
      addDebugMessage(`Fetcher data received: success=${fetcher.data.success}, message=${fetcher.data.message}, error=${fetcher.data.error}`);
      
      if (fetcher.data.success && fetcher.data.savedData) {
        addDebugMessage(`資料成功儲存 (ID: ${fetcher.data.id})`);
        ...
        if (wsStatus === 'connected') {
          sendMessage({
            ...
            data: {
              id: fetcher.data.id,
              content: fetcher.data.savedData,
              scannedAt: new Date().toISOString()
            }
          });
          addDebugMessage('WebSocket notification sent');
        } else {
          addDebugMessage('WebSocket not connected, notification not sent', true);
        }
      } else if (fetcher.data && !fetcher.data.success && fetcher.data.error) {
        addDebugMessage(`儲存資料失敗: ${fetcher.data.error}`, true);
      }
    }
  }, [fetcher.data, addDebugMessage, wsStatus, sendMessage]);
  ...
  return (
    <div className="flex flex-col items-center space-y-2 w-full">
      {/* WebSocket Status */}
      <div className="w-full max-w-sm mb-2 flex items-center justify-between p-2 bg-slate-900 border border-slate-700 rounded-md">
        <span className="text-xs text-slate-400">WebSocket 狀態:</span>
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs text-slate-300">
            {wsStatus === 'connected' ? '已連接' : wsStatus === 'connecting' ? '連接中' : '未連接'}
          </span>
        </div>
      </div>
