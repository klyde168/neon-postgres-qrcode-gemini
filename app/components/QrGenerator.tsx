...
const parseQrCodeTimestamp = (data: string): Date | null => {
  try {
    // 嘗試解析 UUID 格式中的時間戳（如果是時間戳格式的 UUID）
    // 但實際上 UUID 不包含時間信息，所以我們返回當前時間
    
    // 檢查是否為 ISO 8601 格式的時間字串
            ...
            <div className="mt-4 p-3 bg-slate-600 rounded-md text-sm text-slate-300">
              <div className="grid grid-cols-1 gap-2">
                <div>
                  ...
                  <span className="text-slate-200">{timeInfo.scanTime.toLocaleString('zh-TW')}</span>
                </div>
                <div>
                  <span className="text-slate-400">掃描後經過：</span>
                  <span className="text-slate-200">{timeInfo.elapsedSeconds} 秒</span>
                </div>
                {timeInfo.qrCodeTime && timeInfo.timeDifference !== null && (
                  <>
                    <div>
                      <span className="text-slate-400">QR Code 時間：</span>
                      <span className="text-slate-200">{timeInfo.qrCodeTime.toLocaleString('zh-TW')}</span>
                    </div>
                    <div>
